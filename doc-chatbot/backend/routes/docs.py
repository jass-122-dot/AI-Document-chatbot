...
def upload_document():
    ...
    try:
        ...
        return jsonify({'message': 'Document uploaded successfully', 'doc_id': doc_id, 'name': original_name}), 201
...
def chat():
    ...
    db = get_db()
    doc = row_to_dict(db.execute("SELECT * FROM documents WHERE id = ? AND user_id = ?", (doc_id, user_id)).fetchone())
    db.close()
...
def delete_document(doc_id):
    ...
    db = get_db()
    try:
        db.execute("DELETE FROM chat_history WHERE document_id = ? AND user_id = ?", (doc_id, user_id))
